        ...
        <>
                    ...
                    {datos.map((reg, index) => (
                        <tr key={reg.id}>
                            {columnas.map((col, pos) => (
                                <td className="py-2 px-4 text-sm text-gray-500 font-inter" key={pos}>{eval('reg.' + col.key)}</td>
                            ))}
            ...
            <div className="flex justify-center mt-2 bg-gray-100 text-gray-700 font-medium border-b pt-2 pb-2">
                <nav className="inline-flex rounded-md shadow">
                    <button onClick={() => fetchData(page - 1)} disabled={page == 1} className="py-2 px-4 bg-gray-300 text-gray-600 hover:bg-gray-400 rounded-l-md hover:text-gray-100">
                        anterior
                    </button>
                    {total > itemsPerPage && (
                        <div className="flex">
                    ...
                    <button onClick={() => fetchData(page + 1)} disabled={page == Math.ceil(total / itemsPerPage)} className="py-2 px-4 bg-gray-300 text-gray-600 hover:bg-gray-400 rounded-r-md hover:text-gray-100">
                        siguiente
                    </button>
                </nav>
            </div>
