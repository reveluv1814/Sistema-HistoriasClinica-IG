...
const MaxManForm = ({ expFisicaId }) => {
  ...
  const getApartado = async () => {
    try {
      ...
      setDatos(exploracionFFetch.data.maxMandibula);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handlePost = async (values) => {
    try {
      ...
      await exploracionFisicaService.guardarMaxMandibula(expFisicaId, {
        maxMandibula: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
      console.log(error);
  ...
  const handlePatch = async (values) => {
    try {
      ...
      await exploracionFisicaService.editarMaxMandibula(datos.id, {
        maxMandibula: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
  ...
  const Formm = ({ isCreate, handleFun }) => {
    //valores iniciales
    const initialValues = {
      hipoplasiaMaxS: datos ? datos.hipoplasiaMaxS ?? "" : "",
      micrognatia: datos ? datos.micrognatia ?? "" : "",
      prognatismo: datos ? datos.prognatismo ?? "" : "",
      retronagtismo: datos ? datos.retronagtismo ?? "" : "",
      obs: datos ? datos.obs ?? "" : "",
    };
