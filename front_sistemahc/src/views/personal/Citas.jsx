...
const Citas = () => {
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-5 bg-cyan-600  dark:bg-cyan-800  rounded-xl mt-5">
        <div
        ...
        >
          {citas.map((cita, index) => (
            <div
              key={index}
              className="bg-sky-100 p-4 rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.4)] hover:bg-slate-100 hover:scale-105 transition-transform cursor-pointer"
            >
              {/* Renderiza el contenido de cada elemento aquí  */}
              <div className="mb-2 grid grid-cols-1 sm:grid-cols-2 ">
                <div>
                  <b className="text-gray-600 text-xl">Fecha:</b>{" "}
                  {new Date(cita.fecha).toLocaleDateString()}
                </div>
                <div>
                  <b className="text-gray-600 text-xl">Hora:</b> {cita.hora.slice(0, 5)}
                </div>
              </div>
              <p className="mb-2">
                <b className="text-gray-800">Paciente:</b>{" "}
                {cita.paciente.persona.nombreCompleto}
              </p>
              <p className="mb-2">
                <b className="text-gray-800">Doctor:</b>{" "}
                {cita.doctor.persona.nombreCompleto}
              </p>
              <p className="italic text-sm">
                <b className="text-gray-700">Creado Por:</b>{" "}
