...
const CuelloForm = ({ expFisicaId }) => {
  ...
  const getApartado = async () => {
    try {
      ...
      setDatos(exploracionFFetch.data.cuello);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handlePost = async (values) => {
    try {
      ...
      await exploracionFisicaService.guardarCuello(expFisicaId, {
        cuello: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
      console.log(error);
  ...
  const handlePatch = async (values) => {
    try {
      ...
      await exploracionFisicaService.editarCuello(datos.id, {
        cuello: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
  ...
  const Formm = ({ isCreate, handleFun }) => {
    //valores iniciales
    const initialValues = {
      corto: datos ? datos.corto ?? "" : "",
      quistes: datos ? datos.quistes ?? "" : "",
      fistula: datos ? datos.fistula ?? "" : "",
      colli: datos ? datos.colli ?? "" : "",
      torticolis: datos ? datos.torticolis ?? "" : "",
      tiroides: datos ? datos.tiroides ?? "" : "",
      obs: datos ? datos.obs ?? "" : "",
    };
