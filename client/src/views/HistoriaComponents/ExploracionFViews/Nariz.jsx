...
const Nariz = ({ nariz }) => {
  ...
  const keyMappings = {
    puentenasal: "Puente nasal bajo:",
    stenosis: "Estenosis de coanas:",
    tabique: "Desviación del Tabique:",
    hipoplasis: "Hipoplasia alar:",
    narizProminente: "Nariz prominente:",
  ...
  return (
    <>
      <div className="flex flex-row flex-wrap">
        {Object.keys(nariz).map((key) => {
          if (key === "id" || key === "createdAt") {
            return null; // Ignorar las claves "id" y "createdAt"
          }
          const styleP = key === "tipoNariz" || key === "obs" ? true : false;

          const displayName = keyMappings[key] || key;

          const renderContent =
            key === "stenosis" || key === "tipoNariz" || key === "obs" ? (
              nariz[key] || "sin dato..."
            ) : nariz[key] !== null ? (
              nariz[key] ? (
