...
const AbdomenForm = ({ expFisicaId }) => {
  ...
  const getApartado = async () => {
    try {
      ...
      setDatos(exploracionFFetch.data.abdomen);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handlePost = async (values) => {
    try {
      ...
      await exploracionFisicaService.guardarAbdomen(expFisicaId, {
        abdomen: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
      console.log(error);
  ...
  const handlePatch = async (values) => {
    try {
      ...
      await exploracionFisicaService.editarAbdomen(datos.id, {
        abdomen: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
  ...
  const Formm = ({ isCreate, handleFun }) => {
    //valores iniciales
    const initialValues = {
      herniaUmbilical: datos ? datos.herniaUmbilical ?? "" : "",
      herniaInguinal: datos ? datos.herniaInguinal ?? "" : "",
      diastasis: datos ? datos.diastasis ?? "" : "",
      tumoraciones: datos ? datos.tumoraciones ?? "" : "",
      semiologia: datos ? datos.semiologia ?? "" : "",
      obs: datos ? datos.obs ?? "" : "",
    };
