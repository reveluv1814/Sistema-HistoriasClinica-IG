...
const ColumnaForm = ({ expFisicaId }) => {
  ...
  const getApartado = async () => {
    try {
      ...
      setDatos(exploracionFFetch.data.columna);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handlePost = async (values) => {
    try {
      ...
      await exploracionFisicaService.guardarColumna(expFisicaId, {
        columna: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
      console.log(error);
  ...
  const handlePatch = async (values) => {
    try {
      ...
      await exploracionFisicaService.editarColumna(datos.id, {
        columna: valoresNoVacios,
      });
      getApartado();
    } catch (error) {
  ...
  const Formm = ({ isCreate, handleFun }) => {
    //valores iniciales
    const initialValues = {
      cifosis: datos ? datos.cifosis ?? "" : "",
      escoliosis: datos ? datos.escoliosis ?? "" : "",
      lordosis: datos ? datos.lordosis ?? "" : "",
      apendice: datos ? datos.apendice ?? "" : "",
      fovea: datos ? datos.fovea ?? "" : "",
      obs: datos ? datos.obs ?? "" : "",
    };
